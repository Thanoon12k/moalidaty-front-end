...
import 'package:moalidaty/features/workers/api/login_api.dart';
import 'package:moalidaty/features/workers/controllers/worker_preferences_manager.dart';
...
class WorkerLoginController extends GetxController {
  ...
  Future<bool> saveWorkertoPreference() async {
    if (await WorkerPreferencesManager().saveWorkerAccount(myworker!)) {
      debugPrint('welcome worker it is your app  ${myworker!.username} ');
      GlobalConstants.AccountType = "worker";
      GlobalConstants.accountID = myworker!.id!;
  ...
  Future<MyWorker?> loginWorker() async {
    if (formKey.currentState!.validate()) {
      formKey.currentState!.save();

      myworker = await WorkerLoginApi().apiLoginWorker(
        '$identifier',
        '$password',
      );

      return myworker; //user not logged in
    }
    return null;
  }
